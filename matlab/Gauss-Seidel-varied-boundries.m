...
T(Ny+1, :) = T2;                     % Top boundary (constant 100°C)

% Bottom boundary: Linear variation from left (x=0) to right (x=Lx)
for i = 1:Nx+1
    T(1, i) = T1 + (i-1)/Nx * (T2 - T1);  % Varies from T1 to T2 across bottom
end
